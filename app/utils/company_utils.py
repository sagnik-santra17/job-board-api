...
async def get_and_validate_company_ownership(repo, company_id: int, manager_id: int) -> Companies:
    ...
    validate_company_id_exists(company)  # -> raises 404 if the company does not exist or invalid company id

    if company.manager_id != manager_id:
        # User is authenticated but not authorized to access this company
        raise HTTPException(
